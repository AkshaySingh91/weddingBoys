...
export function Signup() {
    ...
    const handleAvatar = (e) => {
        ...
        if (file) {
            ...
            reader.onload = () => {
                ...
                instruct.current.style.display = `none`
    ...
    const handleForm = async (e) => {
        ...
        if (!validateName()) {
            return await fireMessage('INVALID NAME', 'error')
        ...
        try {
            const res = await fetch('http://localhost:5000/admin/verify-details', {
                method: 'POST',
                body: JSON.stringify({
            ...
            if (res.status >= 300 || !data.putUrl || !data.avatarKey) {
            ...
            await fireMessage(error.message)
        }
    }
    const startTimer = () => {
    ...
    const handleVerifySmsOtp = async () => {
        ...
        try {
            ...
            if (res.status !== 200) {
                ...
                // now all details submited by user are verified incl otp & code now we have to save his details inc avatar on s3
                await saveDetails();
            }
        } catch (error) {
            await fireMessage(error.message, 'error')
    ...
    const saveDetails = async () => {
        // name, phone, email, avatar, password
        const uploadFileInBucket = async (file, presignedUrl) => {
            if (!file || !presignedUrl) {
                throw new Error(`Failed to upload file : file required`)
            }
            try {
        ...
        try {
            ...
            if (res.status >= 300) {
                await fireMessage(admin.message, 'error')
            } else {
                clearInterval(intervalId);
            ...
            <div className="bg-white shadow-lg rounded-lg p-6 max-w-md w-full">
                ...
                <form method='POST' className="space-y-4" onSubmit={handleForm} encType="multipart/form-data">
                    <div className="flex flex-col items-center">
                        <div
                            className="thumbnail border-spacing-11 bg-cover bg-center w-28 h-28 rounded-full border-dashed border-2 border-blue-200 flex items-center justify-center cursor-pointer relative overflow-hidden"
                            onClick={() => document.getElementById('img').click()}
                            ref={avatarRef}
                        >
                            <span ref={overlay} className="text-sm text-blue-500">Avatar</span>
                            <input
                                className="hidden"
                                type="file"
                                id="img"
                        ...
                        <span ref={instruct} className="text-xs text-blue-500 my-2">Upload Your Image</span>
                    </div>

                    <div>
                        <label htmlFor="name" className={`block text-sm font-medium after:content-["*"] after:text-red-600 text-gray-700`}>Enter Name</label>
                        <input
                            value={name}
                            type="text"
                            id="name"
                    ...
                    <div>
                        <label htmlFor="code" className={`block text-sm font-medium after:content-["*"] after:text-red-600 text-gray-700`}>Code</label>
                        <input
                            value={code}
                            type="text"
                            id="code"
                    ...
                    <div>
                        <button type="submit" className="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 focus:outline-none"
                        >Signup</button>
                    </div>
                </form>
