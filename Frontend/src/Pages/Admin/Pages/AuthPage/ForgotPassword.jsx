    ...
    const handleForm = async (e) => {
        ...
        try {
            ...
            if (res.status >= 300) {
                ...
                // Details verified; proceed with sending OTP via email only
    ...
    const handleVerifyEmailOtp = async () => {
        ...
        try {
            ...
            if (res.status !== 200) {
                ...
                // Correct OTP received; update UI accordingly
                ...
                // OTP verified via email; proceed to allow password update.
    ...
    const handleChangePassword = async () => {
        ...
        try {
            const res = await fetch(`${api_url}/api/admin/update-password`, {
                method: 'POST',
                body: JSON.stringify({
                    phone, email, password, type: "email"
                }),
                headers: {
                    'content-type': 'application/json'
                }
        ...
        <>
            <div className="min-h-screen flex items-center justify-center bg-gray-100">
                <div className="bg-white shadow-lg rounded-lg p-6 max-w-md w-full">
                    <h1 className="font-bold lg:text-desktopHeadlineSmall sm:text-mobileHeadlineLarge text-center">Wedding Boys</h1>
                    <h2 className="text-2xl font-bold text-center mb-6">Forgot Password</h2>
                    <form onSubmit={handleForm} method='POST' className="space-y-4">
                        <div>
                            <label htmlFor="phone" className="block text-sm font-medium after:content-['*'] after:text-red-600 text-gray-700">
                                Phone
                            </label>
                            <input
                                value={phone}
                        ...
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium after:content-['*'] after:text-red-600 text-gray-700">
                                Email
                            </label>
                            <input
                                value={email}
                    ...
                    <div className="text-center mt-4">
                        <p className="text-sm text-gray-600">
                            Already have an account? <Link to="/admin/login" className="text-blue-600 font-medium hover:underline">Login</Link>
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        {!isAllOtpVerified && (
                            <div className="content" ref={otpInputComponentChildRef}>
                                ...
                                <p className='text-gray-500 px-2 py-4 text-center'>
                                    We've sent verification code to <b>{email}</b>
                                </p>
                                <div className="flex flex-row items-center justify-between mx-auto w-full max-w-xs">
                                    {otpInputRef.map((ref, idx) => (
                                        <input
                                            ...
                                            onPaste={handlePaste}
                                            className="w-10 h-10 text-center p-1 outline-none rounded-xl border border-gray-200 text-lg bg-white focus:bg-gray-50 focus:ring-1 ring-blue-700"
                                            type="text"
                                            required
                                        />
                                    ))}
                                ...
                                <div className="timer text-center mx-auto my-2">
                                    {otpTimer && <p className='text-green-800 tracking-widest font-bold'>{otpTimer}</p>}
                                </div>
                                {!otpTimer && (
                                ...
                                <div className="button-wrapper text-center pt-4 pb-2">
                                    <hr className='border border-gray-600 mx-auto' />
                                    <div className="mt-4 btn-wrapper flex items-center justify-evenly">
                                        <button
                                            onClick={() => {
                                                clearInterval(intervalId)
                                                setIsDetailsVerified(false);
                                            }}
                                            className="my-2 px-8 py-2 border border-gray-800 bg-white rounded-lg"
                                        >
                                            Cancel
                                        </button>
                                        <button
                                            onClick={handleVerifyEmailOtp}
                                            className="my-2 px-8 py-2 text-white border border-gray-800 bg-blue-500 rounded-md"
                                        >
                                            Verify
                                        </button>
                                    </div>
                        ...
                        {isAllOtpVerified && (
                            <div className="content" ref={otpInputComponentChildRef}>
                                ...
                                <div className="mx-auto w-full max-w-xs">
                                    <div>
                                        <label htmlFor="password" className="block text-sm font-semibold after:content-['*'] after:text-red-600 text-gray-700">
                                            Password
                                        </label>
                                    ...
                                    <div>
                                        <label htmlFor="confirm-password" className="block text-sm font-semibold after:content-['*'] after:text-red-600 text-gray-700">
                                            Confirm Password
                                        </label>
                                        <div className="input relative">
                                            <input
