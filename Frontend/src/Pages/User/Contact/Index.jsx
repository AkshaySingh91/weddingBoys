    ...
    <>
      <section className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <div className=" relative grid grid-cols-1 md:grid-cols-2 gap-8 " >
          ...
          <form className="p-6 space-y-6 sm:order-first lg:order-last rounded-2xl sm:bg-[#f5f0e7]" >
            ...
            <div>
              ...
              <input value={reach} type="text" id="source" placeholder='Google, Instagram, Referal, etc'
                className="p-2 mt-1 block w-full border-[1px] border-slate-300 bg-[#fffdfa]  rounded-md shadow-sm outline-none   focus:border-[1px] focus:border-gray-500  sm:text-sm"
                onChange={(e) => setReach(e.target.value)} />
            </div>

            <div className="mt-6 text-center">
              <button type="button" className="px-6 py-2 text-white bg-tertiary hover:bg-tertiary_on rounded-md shadow-sm"
        ...
        <div className="fixed lg:bottom-4 lg:right-4 sm:bottom-10 sm:right-4 z-50">
          ...
          <Link
            to={`https://wa.me/${studioContact ? (Array.isArray(studioContact) ? studioContact[0] : "") : ""}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center  py-3  px-4    bg-gradient-to-r from-[#f7d4a3] to-[#ffdcace2] rounded-2xl shadow-lg transition-transform hover:scale-110 border-[1px] border-[#ffa200]"
            title="Chat with us on WhatsApp">
            <svg className='w-8 h-10 fill-[#ffa200]' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 308 308"  >
